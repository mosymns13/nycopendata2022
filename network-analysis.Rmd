...
Next, we generate a list of individuals that contributed to more than one campaign during the 2021 election cycle. We then filter the simplified contributions data frame to only include these "double dippers" and only look at relatively large donations (an arbitrary threshold we set at $1000).
...
Now we can create a simple network graph visualizing instances when two candidates received donations of at least $1000 from the same campaign contributor.
